...
from configs.config import get_logger, create_parser, protocol_dict
...
def main(args):
    ...
    if args.pc_model == 'pointnet':
        ...
        pointnet2_pretrained = PointNetClassifier(num_points=NUM_POINTS) 
    ...
    elif args.pc_model == 'pointnet2':
        pointnet2_pretrained = PointNet2(num_class=40) 
    ...
    elif args.pc_model == 'pointnet2_simpleview':
    ...
    logger = get_logger(filename = args.pc_model, protocol = protocol_num)
    logger.info(f"training {args.pc_model} for protocol {protocol_num}")
    ...
    for color_imgs, depth_imgs, label in tqdm(train_dataset, desc="training first trainds"):
        ...
        for i in range(color_imgs.size(0)):
            ...
            if args.pc_model == 'pointnet2_simpleview':
    ...
    logger.info(f"Class Distribution: {str(label_counts)}")

    logger.info(f"Features length: {len(features)}")
    logger.info(f"Labels length: {len(labels)}")
    concatenated_features = torch.stack(features, dim=0) 
    concatenated_labels = torch.stack(labels, dim=0)     
    ...
    logger.info(f"{args.pc_model}_{protocol_num} + SVM Accuracy: {accuracy}")
    logger.info("Calculating scores")

    genuine_scores, imposter_scores = [], []
    genuine_path, imposter_path = f"scores/Protocol_{protocol_num}/{args.pc_model}/{args.testds}/lmaubo/genuine.npy", f"scores/Protocol_{protocol_num}/{args.pc_model}/{args.testds}/lmaubo/imposter.npy" 
    os.makedirs(f"scores/Protocol_{protocol_num}/{args.pc_model}/{args.testds}/lmaubo/", exist_ok = True)
