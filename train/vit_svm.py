...
for color_imgs, depth_imgs, label in tqdm(train_dataset, desc="Training first train dataset"):
...
for color_imgs, depth_imgs, label in tqdm(train_dataset2, desc="Training second train dataset"):
    color_imgs, depth_imgs = color_imgs.to(device), depth_imgs.to(device)
    label_class = label.argmax(dim=1).detach().to(device)
...
for color_imgs, depth_imgs, label in tqdm(test_dataset, desc="First test dataset"):
    color_imgs, depth_imgs = color_imgs.to(device), depth_imgs.to(device)
    label_class = label.argmax(dim=1).detach().to(device)
...
for color_imgs, depth_imgs, label in tqdm(test_dataset2, desc="Second test dataset"):
    color_imgs, depth_imgs = color_imgs.to(device), depth_imgs.to(device)
    label_class = label.argmax(dim=1).detach().to(device)
...
print("ViT + SVM Accuracy:", accuracy)

print("Calculating scores")
genuine_scores, imposter_scores = [], []
genuine_path, imposter_path = "scores/vit_svm/iPhone12_filled/lmaubo/genuine.npy", "scores/vit_svm/iPhone12_filled/lmaubo/imposter.npy"
os.makedirs("scores/vit_svm/iPhone12_filled/lmaubo/", exist_ok=True)
