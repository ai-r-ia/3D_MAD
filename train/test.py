...
def compute_eer(models, morph_types, root_dir, args, logger):
    ...
    for morph_type in morph_types:
        ...
        save_path = f"scores/eer_table_{dataset_name}_params_ablation.pkl" #Note: change based on experiment
...
def main(args):
    ...
    protocol_num = protocol_dict[f"{args.trainds}_{args.testds}"]
    logger = get_logger(filename = "scores_params_ablation", protocol = protocol_num) #Note: change based on experiment
    ...
    for reduction in reductions:
        for kernel_size in kernel_sizes: 
            ...
            model_name = f"{model_name}_{args.trainds}_{reduction}_{kernel_size}"
